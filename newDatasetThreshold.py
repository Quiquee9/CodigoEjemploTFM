...
if __name__ == '__main__':
    ...
    listThreshold=[0.9,0.8,0.7,0.6,0.5,0.4,0.3,0.2,0.1]
    ...
    for threshold in listThreshold:
        ...
        for idVideo in namesUniques:
            ...
            #Si esta vacio de añade el frame con el posterior mayor
            if(dfVid.loc[dfVid.loc[:, "embs_" + str(realEmotion)] >= threshold].empty):
                #indice del maximo frame del video
                if(incluirMaximoTrain==1):
                    maximoVideo=dfVid[["embs_" + str(realEmotion)]].idxmax()[0]
                    dfThreshTrain = dfThreshTrain.append(dfVid.loc[maximoVideo])
                #Cuento cuanto pasa por video y emocion
                contador_missPerEmot[realEmotion]=contador_missPerEmot[realEmotion]+1
        ...
        for emotion in range(7):
            framesEmotion = len(df_Posteriors.loc[df_Posteriors.loc[:, 'emotion'] == emotion].index)
            framesEmotion_filtTrain = len(dfThreshTrain.loc[dfThreshTrain.loc[:, 'emotion'] == emotion].index)
            framesEmotion_filtTest = len(dfThreshTest.loc[dfThreshTest.loc[:, 'emotion'] == emotion].index)
            print("Emotion: "+emot_dict2[emotion])
            print("Frames totales: "+str(framesEmotion)+"| Frames filtrados:  "+str(framesEmotion_filtTrain)+
                  "| Videos que no superan el threshold en ningún frame: "+str(contador_missPerEmot[emotion])+"/"+numVideosPerEmotion[emotion])
        ...
        path="/home/enriquediez/Guided-EMO-SpatialTransformer-main/data/datasets_distribution/RADVESSthresholdTRAIN2"
        ...
        path="/home/enriquediez/Guided-EMO-SpatialTransformer-main/data/datasets_distribution/RADVESSthresholdTEST2"
        os.makedirs(path, exist_ok=True)
