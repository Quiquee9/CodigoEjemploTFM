...
def get_embs_5CV(k_folds,test_dataset, batch_size, root_path_weights, modality, logs_path, n_classes_preTrained, type_emb):
    ...
    #Select data to test
    print("ACTORS IN TEST: ALL")
    test_idx = np.array(
        list(test_dataset.df_file.loc[test_dataset.df_file["actor"].isin(list(range(0, 24)))].index))

    # GET SAMPLES FROM ACTORS:
    print('--------------------------------')
    test_subsampler = torch.utils.data.SubsetRandomSampler(test_idx)

    testloader = DataLoader(test_dataset, batch_size=batch_size, sampler=test_subsampler, num_workers=6,pin_memory=True)
    #Load weights
    root_w_path = root_path_weights

    print("Loaded weight: ", root_w_path)
    #Eval model:
    fold_logs_path = os.path.join(logs_path, "fold"+str(0))
    os.makedirs(fold_logs_path, exist_ok=True)


    df_embs = extract_embs(modality, root_w_path, testloader, test_dataset, fold_logs_path, type_emb, n_classes=n_classes_preTrained)
